          ...
          <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 p-6 space-y-5 animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <h3 className="text-lg font-bold text-slate-900">{applyingJob.title}</h3>
                <p className="text-xs text-slate-500">{applyingJob.companyName} • {applyingJob.location}</p>
              </div>
              <button
            ...
            {!isRegisteredCandidate ? (
              <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 space-y-2">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  To apply for Ayudh Vikas or partner vacancies, candidates must complete the one-time registration form with ₹10 payment verification.
                </p>
                <div className="pt-1">
                  <Link
                  ...
                  >
                    <span>Complete Candidate Registration Now →</span>
                  </Link>
                </div>
              </div>
            ) : (
              <div className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center space-x-2 text-xs text-emerald-800">
                ...
                <span>Verified Candidate Registration Active (₹10 Paid). Ready to apply.</span>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-700">
                Candidate Note / Relevant Experience (Optional)
              </label>
              <textarea
                rows={3}
                placeholder="Briefly state your relevant skills, certifications, and availability..."
                value={applyNotes}
            ...
            <div className="flex items-center justify-end space-x-3 pt-2">
              ...
              <button
                disabled={applySubmitting || !isRegisteredCandidate}
